 ...
 * 
 *  A Higher Order Functgion(HOF) is a function that either takes a function as an argument OR
 *      returns a function
 */

function timesX(x) {
    //Anonymous Function: it has no name because it needs no name
    return function(number) {
        /**
         * Closure:
         *  - occurs when you have a function inside of another function referring to some variable higher up
         *      in the chain
         *  - it is a function that contains encapsulated references to its surround "Lexical environment"
...
// In JS, Functions are considered "first-class citizens" meaning that they can be stored in variables, put in arrays,
...
// Arrow functions are an easier way of passing anonymous functions
// JS uses => instead of Java ->

//Some rules apply for these arrow functions
 ...
 * 
 * Invalid
 * (num1, num2) => return num1 + num2; // need to surround return statement with {}
 * 
 * Valid:
...
// Write calcSum as an arrow function
// Using arrow functions to write functions is VERY popular in JS development
...
console.log(planets);   // Still the original array
...
const alteredPlanets = planets.map((planet) => {
    return planet.toUpperCase();    //Creates a new aray consisting of the upper cased form of the old array
});

console.log(alteredPlanets);

// Filter "filters" the original array by only adding select elements into the new array
...
//Main difference from Java's is that to specify an initial value, it comes after the function definition
