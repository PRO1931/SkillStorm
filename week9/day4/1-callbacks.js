// Callbacks are the old school way of handling asynchronous code in JS

// Instead of twiddling our thumbs waiting for a request that we sent off to comeback, we do other things
//  and await a specific event to occur before coming back to handle it
 ...
 * 
 *  We want to request soome data from a web server to load up onto our page
 *  1. We must make the request to the web server
 *  2. We must wait for the response from the web server with our data
 *  3a. If it's a succesful response (we got our data), parse the data and display to the page
 *  3b. If not successful, display an error message to the user
 ...
 *  6b. If unsuccessful, display an error message on the screen
 */

// To simulate the wait time(latency) of a request to a webserver, I will use setTimeouot
...
setTimeout(() => {
    ...
    if (data) {
        ...
        button.addEventListener('click', () => {
            ...
            }, 5000);   // Timeout for 5 sec...before doing above activity
        });
    } else {
