...
public class DatePractice {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * Most of the util Date library has been deprecated: support has been pulled and its on End of Life
		 * 
		 ...
		 * 5. Overflows days in a month (Feb29th on nonleap year goes to March 1st)
		 * 
		...
		System.out.println(yesterdaysDate.after(todaysDate));
	...
	//	System.out.println(epochDate);
		...
		String  todayAsYYYYMMDDString = new SimpleDateFormat("yyy-mm-dd").format(todaysDate);
...
//		}
		 ...
		 * 1. I can handle dates in YYY-MM-DD out of the box
		...
		System.out.println("Yesterday's date as a Local Date: " +  yesterdaysLocalDate);
		
		
		//Create a LocalDate using a String
		...
		System.out.println("March 15, 2022 as a LocalDate: " + march152022);
		...
		try {
			...
			System.err.println("Error when parsing date String");
		}
		
		//Attempting an invalid leap year date
		try {
			LocalDate invalidDate = LocalDate.parse("2022-02-2029");
			System.out.println("Printing the invalid date: " + invalidDate);
		} catch (DateTimeParseException e) {
		...
		try {
			Thread.sleep(1000);  // Tells the operating system to come back to execute in 1000 milliseconds (or 1 sec)
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
		...
		System.out.println("The currentday and time is: " +todaysDateAndTime);
		...
		System.out.println("Combining the date and time yields: " + todaysLocalDateTime);
		
		
		System.out.println("A person born in 1923-05-26 is " + calculateAge(LocalDate.of(1923, 5, 26)) + " years old.");
	}
